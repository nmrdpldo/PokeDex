...
import com.rodsamples.pokedex.utils.Constants.PAGE_SIZE
...
class PokemonListViewModel @Inject constructor(
    ...
    fun searchPokemonList(){
        ...
        viewModelScope.launch(Dispatchers.Default) {
            ...
            val results = listToSearch.filter {
                it.pokemonName.contains(query.value.trim(), ignoreCase = true) ||
                        it.number.toString() == query.value.trim()
            }
            if(isSearchStarting){
    ...
    fun loadPokemonPaginated(){
        viewModelScope.launch {
            ...
            when (result) {
                is Resource.Success -> {
                    endReached.value = currPage * PAGE_SIZE >= result.data!!.count // to check if we're loading more entries than actual data count
